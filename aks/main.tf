...
locals {
  ...
  oms_agent_enabled =  var.loganalytics_workspace_name != null ? (data.azurerm_log_analytics_workspace.this.0.id != null ? true : false) : false  #data.azurerm_log_analytics_workspace.this.0.id
}
...
resource "azurerm_kubernetes_cluster" "this" {
  ...
  dynamic "default_node_pool" {
    for_each = tolist([each.value["aks_default_pool"]])
    content {
  ...
  addon_profile {
    oms_agent {
      ...
      log_analytics_workspace_id = local.oms_agent_enabled == true ? data.azurerm_log_analytics_workspace.this.0.id : null #data.azurerm_log_analytics_workspace.this.id
    }
  ...
  network_profile {
    ...
    outbound_type      = var.userDefinedRouting
    ...
    dynamic "load_balancer_profile" {
      for_each = lookup(each.value, "load_balancer_profile", null) != null ? tolist([each.value.load_balancer_profile]) : []
      content {
  ...
  dynamic "role_based_access_control" {
    for_each = tolist([coalesce(each.value.rbac_enabled, false)])
    content {
      enabled = role_based_access_control.value
      dynamic "azure_active_directory" {
        for_each = var.ad_enabled != false ? tolist([var.ad_enabled]) : []
        content {
  ...
  dynamic "auto_scaler_profile" {
    for_each = lookup(each.value, "auto_scaler_profile", null) != null ? tolist([each.value.auto_scaler_profile]) : []
    content {
...
resource "azurerm_key_vault_access_policy" "this" {
  count                   = length(local.msi_enabled_aks_clusters)
  key_vault_id            = data.azurerm_key_vault.this.id
